...
{ 
  ...
  for (j=0;j<n;j++){
    plist[j].wt = (plist[j].tat - plist[j].bt);
  }
  k = 0;
  for (j=0;j<n;j++){
    if (rem_bt[j] > quantum){
...
{ 
  ...
  int j, compCounter, compTime, lapNum;
  compCounter = 0;
  compTime = 0;
  lapNum = 0;
  
  for ((j=0;j<n;j++)){
    plist[j].wt = ((plist[j].wt) -1);
    if (plist[j].wt == 0){
      compCounter++;
      lapNum++;
      compTime = compTime+1;
      plist[j].wt = ((compTime - plist[j].art - plist[j].bt));
    }
  }
} 
...
{ 
    ...
    for (int  j = 1; j < n ; j++ ) 
        plist[j].wt =  plist[j-1].bt + plist[j-1].wt; 
} 
...
{ 
    ...
    for (int  j = 0; j < n ; j++) 
        plist[j].tat = plist[j].bt + plist[j].wt; 
} 
